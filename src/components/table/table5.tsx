...
const Card: React.FC<CardProps> = ({ data, columns }) => {
  ...
  const sortedEventHistory = [...data]
    .filter(
      (row) =>
        (!filterDate || row.registrationDate.includes(filterDate)) && (!filterStatus3 || row.status2 === filterStatus3),
    )
    .sort((a, b) => {
  ...
  const status3Options = Array.from(new Set(data.map((row) => row.status2)));
